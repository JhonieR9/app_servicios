...
self.addEventListener('install', event => {
  event.waitUntil(
    Promise.all([
      ...
      caches.open(CACHE_NAME).then(cache => cache.addAll(PAGE_ASSETS).catch(() => {})),
...
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',sans-serif;background:#0a0a0f;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;color:white}.card{background:#111118;border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:48px 32px;max-width:400px;width:100%;text-align:center;box-shadow:0 32px 64px rgba(0,0,0,0.5)}.icon{font-size:3.5rem;margin-bottom:16px;display:block}h1{font-size:1.4rem;font-weight:800;margin-bottom:10px}p{color:#64748b;font-size:0.9rem;line-height:1.6;margin-bottom:28px}button{background:linear-gradient(135deg,#4f46e5,#7c3aed);color:white;border:none;padding:14px 28px;border-radius:12px;font-size:0.95rem;font-weight:700;cursor:pointer;width:100%}</style>
</head><body><div class="card"><span class="icon">📡</span><h1>Sin conexión</h1><p>Verifica tu conexión e intenta de nuevo.</p><button onclick="location.reload()">🔄 Reintentar</button></div></body></html>`,
...
self.addEventListener('push', event => {
  let data = { title: '🔔 TalentHub', body: 'Tienes una nueva notificación', url: '/trabajador/panel' };
  try {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title, {
      ...
      tag:     'talenthub-nueva-solicitud',   // reemplaza la anterior si ya existe
      renotify: true,
      vibrate: [200, 100, 200],
      data:    { url: data.url || '/trabajador/panel' }
    })
  );
});
